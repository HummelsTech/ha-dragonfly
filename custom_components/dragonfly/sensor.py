"""Sensor platform for the Dragonfly Shipping parcel tracker integration."""
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: DragonflyConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Dragonfly sensor entities from a config entry."""
    ...
    for parcel in coordinator.data or []:
        entities.append(
            DragonflyParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(DragonflyNextDeliverySensor(coordinator, entry))
    entities.append(DragonflyDeliveredParcelsSensor(coordinator, entry))
    entities.append(DragonflyLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class DragonflyIncomingParcelsSensor(
    CoordinatorEntity[DragonflyCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                DragonflyParcelSensor(self.coordinator, self._entry, barcode)
...
class DragonflyParcelSensor(CoordinatorEntity[DragonflyCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Dragonfly parcel."""
    ...
    def __init__(
        self, coordinator: DragonflyCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        super().__init__(coordinator)
        self._entry = entry
...
class DragonflyNextDeliverySensor(
    CoordinatorEntity[DragonflyCoordinator], SensorEntity
):
...
class DragonflyDeliveredParcelsSensor(
    CoordinatorEntity[DragonflyCoordinator], SensorEntity
):
    """Recently delivered tracked Dragonfly parcels."""
...
class DragonflyLastUpdateSensor(
    CoordinatorEntity[DragonflyCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Dragonfly was last polled successfully."""
